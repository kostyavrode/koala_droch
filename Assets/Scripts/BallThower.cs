...
public class BallThower : MonoBehaviour
{
    ...
    void PickupBall()
    {
        ...
        Ball.transform.localPosition = Vector3.Lerp(Ball.transform.localPosition, Ball.transform.localPosition, 80f * Time.deltaTime);
    }
    ...
    private void Update()
    {
        ...
        else if (Input.GetMouseButtonUp(0))
        {
            ...
            if (swipeTime < 0.5f && swipeDistance > 30f)
            {
                ...
                CalSpeed();
                CalAngle();
                rb.AddForce(new Vector3((angle.x*2 * BallSpeed), (angle.y * BallSpeed / 2), (-angle.z * BallSpeed)));
                rb.useGravity = true;
                holding = false;
                thrown = true;
                Invoke("ResetBall", 3f);
